...
def separate(Xy):
    ...
    participants_train = list(set(participants) - set(participants_test))
    ...
    for participant in participants_test:
        df_g = Xy[Xy.participant == participant].groupby(['h_structure']).mean()
    ...
    list_Xy = [Xy[~indexes_test], Xy[indexes_test], Xy_train_gr, Xy_test_gr]  
    return list_Xy
